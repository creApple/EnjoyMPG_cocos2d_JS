...
var GameLayer = cc.Layer.extend({
    ...
    init:function () {
    	...
    	GAME.CONTAINER.BALLS = [];
    	GAME.CONTAINER.BLOCKS = [];
    	GAME.LIFE = 4;
    	GAME.BLOCKCOUNT = 0;
    	this._state = STATE_PLAYING;
    	this._tmpScore = GAME.SCORE;

    	winSize = cc.winSize;
     	this.screenRect = cc.rect(0, 0, winSize.width, winSize.height + 10);
 	
    	// score
    	this.lbScore = new cc.LabelTTF("Score: " + GAME.SCORE, "Arial", 20);
    	...
    	for (var i = 0; i < this._mapHeight; i++) {
    		for (var j = 0; j < this._mapWidth; j++) {
    			if (GAME.LEVEL%2) {
    				var tileNumber = GAME.MAP_STAGE1[n];                   
    			} else {
    				var tileNumber = GAME.MAP_STAGE2[n];                      
    			}
    			n++;
    			if(tileNumber) {
    ...
    checkBatCollide:function () {
        var ballChild;
        // check collide
        for (var i = 0; i < GAME.CONTAINER.BALLS.length; i++) {
            ballChild = GAME.CONTAINER.BALLS[i];
    ...
    checkBlockCollide:function () {
        ...
        for (var i = 0; i < GAME.CONTAINER.BALLS.length; i++) {
            ballChild = GAME.CONTAINER.BALLS[i];
            for (var j = 0; j < GAME.CONTAINER.BLOCKS.length; j++) {
                blockChild = GAME.CONTAINER.BLOCKS[j];
    ...
    checkIsReborn:function () {
        if (GAME.LIFE > 0 && !this._bat.active) {
			this._bat.born();
        }
        else if (GAME.LIFE <= 0 && !this._bat.active) {
    ...
    updateUI:function () {
        ...
        this.lbLife.setString(GAME.LIFE + '');
    ...
    initBackground:function () {
    	this._backSky = new cc.Sprite("#gameBg.png");
    	this._backSky.attr({
    		anchorX: 0,
    		anchorY: 0,
    		x: 0,
    ...
    movingBackground:function(dt){
    	...
    	if (this._backSkyHeight <= winSize.height) {
    		if (!this._isBackSkyReload) {
    			this._backSkyRe = new cc.Sprite("#gameBg.png");
