...
var GameLayer = cc.Layer.extend({
    ...
    init:function () {
    	...
    	GAME.CONTAINER.HAMMERS = [];
    	GAME.CONTAINER.MOLES = [];
    	GAME.LIFE = 4;
    	GAME.MOLECOUNT = 0;
    	GAME.SCORE = 0;
    	this.resetMole();
    	...
    	// score
    	this.lbScore = new cc.LabelBMFont(GAME.SCORE, res.s_scorefont_fnt, 120);
    	this.lbScore.x = 180;
    	...
    	this._point.attr({
    		opacity: 0,
    		x: GAME.KEEPER.XPOSITION,
    		y: GAME.KEEPER.YPOSITION
    	});
    	...
    	if(GAME.EASYMODE) {
    		...
    		this._basicScore += GAME.LEVEL * 200;
    		...
    		this._basicScore += GAME.LEVEL * 400;
    		this._time -= 10;
    	}
    	
    	// Keeper
    	this._keeper = new Keeper();
    	this._keeper.x = GAME.KEEPER.XPOSITION;
    	this._keeper.y = GAME.KEEPER.YPOSITION;
    ...
    measurePower:function( event ) {
    	if( this._state == STATE_PLAYING ) {    		
    		...
    		this._deltaXvalue = GAME.KEEPER.XPOSITION - Math.round(curPos.x);
    		this._deltaYvalue = GAME.KEEPER.YPOSITION - Math.round(curPos.y);
    		...
    		if(GAME.EASYMODE) {
    			...
    			var xpositionToGround = GAME.KEEPER.XPOSITION + this._deltaXvalue * timeToGround ;
    ...
    checkMoleCollide:function () {
    	...
    	for (var i = 0; i < GAME.CONTAINER.HAMMERS.length; i++) {
    		hammerChild = GAME.CONTAINER.HAMMERS[i];
    		for (var j = 0; j < GAME.CONTAINER.MOLES.length; j++) {
    			moleChild = GAME.CONTAINER.MOLES[j];
    ...
    checkTargetCollide:function () {
    	var moleChild;
    	for (var i= 0;i< GAME.CONTAINER.MOLES.length; i++) {
    		moleChild = GAME.CONTAINER.MOLES[i];
    ...
    generateMole:function () {
    	...
    	if(this._state == STATE_PLAYING &&
    			GAME.MOLE_XPOSITION[arrayIndex][1]  && 
    			GAME.MOLECOUNT < 4) {

    		var mole = Mole.createMole(this.getRandom(1,4),
    				arrayIndex,
    				GAME.MOLE_XPOSITION[arrayIndex][0],
    				GAME.KEEPER.YPOSITION-10);
    		this.addChild(mole);
    		GAME.MOLE_XPOSITION[arrayIndex][1] = false;
    	} 
    },
    resetMole:function () {
    	for (var i = 0; i < GAME.MOLE_XPOSITION.length; i++) {
    		GAME.MOLE_XPOSITION[i][1] = true;
    	}
    },
    ...
    initBackground:function () {
    	// bg 
    	this._backGlass = new cc.Sprite("#glassBg.png");
    ...
    movingBackground:function(dt){
    	...
    	if (this._backSkyWidth <= winSize.width) {
    		if (!this._isBackSkyReload) {
    			this._backSkyRe = new cc.Sprite("#skyBg.png");
    	...
    	if (this._backTileMapWidth <= winSize.width) {
    		if (!this._isBackTileReload) {
    			this._backTileMapRe = new cc.TMXTiledMap(res.s_backGround);
